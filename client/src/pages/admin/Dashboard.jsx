...
const Dashboard = () => {
  ...
  // ✅ Fetch admin dashboard + shows + show details per movie
  const fetchDashboardData = async () => {
    try {
      ...
      if (dashboardRes?.success && moviesRes?.success) {
        ...
        // 3️⃣ For each movie, get first show (price + datetime)
        const moviesWithShows = await Promise.all(
          movies.map(async (movie) => {
            try {
              const { data: showRes } = await axios.get(`/api/shows/${movie._id}`);
              if (showRes?.success && showRes.dateTime?.length > 0) {
                const firstDate = showRes.dateTime[0];
                const firstShow = firstDate.times[0];
              ...
              console.error(`Error fetching show for movie ${movie._id}`, err);
              return movie;
            }
          })
        );
        ...
        setDashboard({
          ...
          totalUsers: dashboardRes.dashboardData?.totalUsers || 0,
          activeShows: moviesWithShows,
        });
      }
  ...
  return (
    <>
      ...
      <p className="mt-12 text-lg font-semibold text-white relative z-10">
        Active Shows
      </p>
